...
public class SecurityResourceFunctionalTesting {
    ...
    private static final String url = "http://localhost:8080/Proyecto1.0.0.1-SNAPSHOT/api/v0";
    ...
    public void testPlayerAnnotationOK() {
        String response = new RestBuilder<String>(url).path(Uris.SECURITY).path(Uris.PLAYERANNOTATION).basicAuth("u1", "123456")
                .clazz(String.class).get().build();
        System.out.println("INFO >>>>> " + response);
    }
    @Test
    public void testPlayerUriOK() {
        String response = new RestBuilder<String>(url).path(Uris.SECURITY).path(Uris.PLAYERURI).basicAuth("u1", "123456")
                .clazz(String.class).get().build();
        System.out.println("INFO >>>>> " + response);
    }
}
